...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setHeaderVisible(
        (isScrolledDown && currentScrollPos > scrollThreshold) ||
          !isScrolledDown
      );
  ...
  return (
    <header
      className={`sticky top-0 z-50  w-full transition-all duration-300 ease-in-out    ${
        isHeaderVisible ? "bg-[#040f21]" : "bg-[#040f21] opacity-0"
      }`}
    >
      <div className="container mx-auto flex items-center justify-between h-16 px-4">
        ...
        <ul
          className={`md:flex md:relative md:w-auto w-full ${
            isMobileMenuOpen ? "flex" : "hidden"
          } md:flex-row flex-col md:items-center md:h-auto h-[180px] absolute md:top-0 top-16 left-0 md:bg-none bg-[#040f21] items-center text-white`}
        >
          <li className="mr-6 md:h-[60px] h-14 border-b md:border-none md:hover:text-red-400 border-red-500 flex items-center">
            <Link onClick={disableMenu} to="/">
              Anasayfa
            </Link>
          </li>
          <li className="mr-6 md:h-[60px] h-14 border-b md:border-none md:hover:text-red-400 border-red-500 flex items-center">
            <Link onClick={disableMenu} to="/favorite">
              Favoriler
            </Link>
          </li>
          <li className="mr-6 md:h-[60px] h-14 border-b md:border-none md:hover:text-red-400 border-red-500 flex items-center">
            <Link onClick={disableMenu} to="/signup">
              Üye ol
            </Link>
          </li>
        </ul>
